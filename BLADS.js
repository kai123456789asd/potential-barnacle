...
if (body) {
  switch (true) {
      ...
      try {
        ...
        if (b.data?.tab) {
          ...
          dataTab.push(
            ...
            { id: 33, name: "\u6211\u7684", uri: "bilibili://user_center/", tab_id: "\u6211\u7684tab", pos: 2 }
          );
          b.data.tab = dataTab;
        }
        if (b.data.top) {
        ...
        if (b.data.bottom) {
          b.data.bottom = b.data.bottom.filter(tab => tab.id === 33); // 仅保留“我的”Tab（假设 id 为 33）
        }
    ...
    case /ecommerce-user\/get_shopping_info\?/.test($request.url):
      try {
        let a = JSON.parse(body);
        a.data && (a.data = { shopping_card_detail: {}, bubbles_detail: {}, recommend_card_detail: {}, selected_goods: {}, h5jump_popup: [] });
        body = JSON.stringify(a);
      } catch (a) {
    ...
    case /^https?:\/\/app\.bilibili\.com\/x\/v2\/search\/square/.test($request.url):
      try {
        let a = JSON.parse(body);
        a.data = { type: "history", title: "\u641C\u7D22\u5386\u53F2", search_hotword_revision: 2 };
        body = JSON.stringify(a);
      } catch (a) {
    ...
    case /pgc\/page\/(bangumi|cinema\/tab\?)/.test($request.url):
      try {
        ...
        a.result.modules.forEach(a => {
          ...
          a.style.startsWith("function") && (a.items = a.items.filter(a => -1 == a.blink.indexOf("bilibili.com")));
